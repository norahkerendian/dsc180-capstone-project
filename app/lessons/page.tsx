...
const defaultStats: QuestionStats = {
  attempts: 0,
  correct: 0,
  level: 2, // start at medium until we learn
};

type StatsMap = Record<string, QuestionStats>;
const STATS_STORAGE_KEY = "capstone-question-stats";

/* Difficulty helpers */

function difficultyScore(stats: QuestionStats): number {
  if (stats.attempts === 0) return 0.5; // unknown → medium
  return 1 - stats.correct / stats.attempts; // 0 = very easy, 1 = very hard
}

function levelFromScore(score: number): DifficultyLevel {
  if (score < 0.33) return 1; // easy
  if (score < 0.66) return 2; // medium
  return 3; // hard
}
...
function updateStats(
  ...
  const base = prev ?? defaultStats;
  const attempts = base.attempts + 1;
  const correct = base.correct + (answeredCorrectly ? 1 : 0);
  const score = difficultyScore({ ...base, attempts, correct });
  const level = levelFromScore(score);
  return { attempts, correct, level };
}
...
export default function LessonsPage() {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    window.localStorage.setItem(STATS_STORAGE_KEY, JSON.stringify(statsById));
  }, [statsById]);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const normalized: NormalizedQuestion[] = raw.map((q) => {
          const options = q.choices.map((choice, idx) => ({
            id: String(idx),
            text: choice,
            isCorrect: idx === q.answerIndex,
          ...
          return {
            id: String(q.id),
            topic: "Level 1", // you can change this label later
            prompt: q.prompt,
            options,
            explanationCorrect: "Correct!",
            explanationIncorrect: q.hint || "Not quite.",
  ...
  // Get stats for the current question (with defaults)
  const stats = statsById[question.id] ?? defaultStats;
  ...
  const handleChoice = (index: number) => {
    ...
    // 🔥 Train difficulty for this question
    setStatsById((prev) => ({
      ...prev,
      [question.id]: updateStats(prev[question.id], isCorrect),
    }));
  };
    ...
    <div className="mx-auto max-w-3xl p-6">
      ...
      <p className="text-sm text-zinc-600">
        ...
        <span className="font-medium">
          Difficulty: Level {stats.level}
        </span>{" "}
        <span className="text-xs text-zinc-500">
          ({stats.correct}/{stats.attempts} correct so far)
        </span>
      </p>

      <section className="mt-4 rounded-xl border bg-white p-4 shadow">
        <p className="text-lg font-semibold">{question.prompt}</p>
